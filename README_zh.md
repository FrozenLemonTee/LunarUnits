...
符号解析放在独立的 `notation/` 层：`notation/catalog` 提供不可变的「符号 → 单位」查找表，且不参与核心单位身份判断；`notation/preset` 为每个单位包提供一份现成目录（外加 `all()` 取并集）。这是未来 parser 和用户自定义单位符号的基础。
