...
// 通过目录按符号解析单位，或直接解析整个表达式。
let catalog = @preset.all()
let newton_unit = catalog.lookup("N").unwrap() // 由符号得到牛顿单位
let accel_unit = @parser.parse_unit(catalog, "m/s^2") // 从字符串组合出单位
let g = @parser.parse_quantity(catalog, "9.8 m/s^2") // 数值 9.8，单位 m/s^2

// 温度是仿射标度：绝对点带 offset 换算。
let body = @affine.Point::new(37.0, @affine.celsius)
let body_k = body.to_base() // 310.15 K，作为线性 Quantity
